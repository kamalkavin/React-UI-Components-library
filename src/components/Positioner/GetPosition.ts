 ...
 */
function getPosition({ dimensions, position, targetOffset, targetRect, viewport, viewportOffset = 8, variant }) {
 ...
 */
function getRect({ dimensions, position, targetOffset, targetRect, variant }) {
	...
	switch (position) {
		case Position.LEFT:
			return makeRect(dimensions, {
				left:
					variant === 'error'
						? targetRect.left - dimensions.width - targetOffset - 8
						: targetRect.left - dimensions.width - targetOffset,
				top: variant === 'error' ? alignedLeftRightY - 2 : alignedLeftRightY,
			});
		case Position.RIGHT:
			return makeRect(dimensions, {
				left: variant === 'error' ? targetRect.right + targetOffset - 4 : targetRect.right + targetOffset,
				top:
					variant === 'error'
						? alignedLeftRightY - 3
						: variant !== 'infotext'
